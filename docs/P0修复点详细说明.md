# P0修复点详细说明

**整理日期**: 2025-11-21  
**目标**: 明确每个P0修复点的具体代码修改

---

## P0-1: 注册流程事务保护

### 修改文件
- `src/main/java/com/tripdog/service/impl/UserServiceImpl.java`

### 具体修改
```diff
+import org.springframework.transaction.annotation.Transactional;

     @Override
+    @Transactional(rollbackFor = Exception.class)
     public UserInfoVO register(UserRegisterDTO registerDTO) {
```

### 修改说明
- **添加**: `@Transactional(rollbackFor = Exception.class)` 注解
- **目的**: 确保注册流程中的所有数据库操作在同一事务中执行
- **影响**: 如果任何步骤失败，整个注册流程回滚

---

## P0-2: 验证码存储迁移到Redis

### 修改文件
- `src/main/java/com/tripdog/service/impl/EmailServiceImpl.java`

### 具体修改
```diff
-import java.util.concurrent.ConcurrentHashMap;
+import com.tripdog.common.RedisService;

-    // 使用内存存储验证码（生产环境建议使用Redis）
-    // Key: 验证码, Value: 验证码信息（包含邮箱和过期时间）
-    private final ConcurrentHashMap<String, CodeInfo> codeStorage = new ConcurrentHashMap<>();
+    private final RedisService redisService;
+
+    /**
+     * 验证码Redis Key前缀
+     */
+    private static final String CODE_KEY_PREFIX = "email:code:";
+    
+    /**
+     * 验证码生成最大重试次数限制（防止无限循环）
+     */
+    private static final int MAX_GENERATE_RETRIES = 100;
```

### 存储逻辑修改
```diff
-        codeStorage.put(code, new CodeInfo(email, System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(CODE_EXPIRE_MINUTES)));
+        String codeKey = CODE_KEY_PREFIX + code;
+        boolean stored = redisService.setString(codeKey, email, CODE_EXPIRE_MINUTES, TimeUnit.MINUTES);
```

### 验证逻辑修改
```diff
-        CodeInfo codeInfo = codeStorage.get(code);
-        if (codeInfo == null) { ... }
-        codeStorage.remove(code);
+        String codeKey = CODE_KEY_PREFIX + code;
+        String storedEmail = redisService.getString(codeKey);
+        if (storedEmail == null) { ... }
+        Boolean deleted = redisService.delete(codeKey);
```

### 修改说明
- **移除**: `ConcurrentHashMap` 内存存储
- **添加**: Redis存储逻辑
- **添加**: 最大重试次数限制
- **添加**: 原子删除操作（防止重放攻击）

---

## P0-3: 文件大小和类型限制

### 新增文件
- `src/main/java/com/tripdog/common/utils/FileValidationUtils.java` (新建)

### 修改文件
- `src/main/java/com/tripdog/controller/DocController.java`
- `src/main/java/com/tripdog/service/impl/ChatServiceImpl.java`
- `src/main/resources/application.yaml`

### application.yaml修改
```yaml
  # 文件上传配置
  servlet:
    multipart:
      # 单个文件最大大小（100MB）
      max-file-size: 100MB
      # 请求最大大小（100MB）
      max-request-size: 100MB
      # 文件写入磁盘的阈值（超过此大小会写入临时文件）
      file-size-threshold: 10MB
```

### DocController修改
```diff
+import com.tripdog.common.utils.FileValidationUtils;

+        // 验证文件大小和类型（在业务层再次验证，确保安全）
+        try {
+            FileValidationUtils.validateDocumentFile(file);
+        } catch (IllegalArgumentException e) {
+            log.warn("文件验证失败: {}", e.getMessage());
+            return Result.error(ErrorCode.PARAM_ERROR, e.getMessage());
+        }
```

### ChatServiceImpl修改
```diff
+import com.tripdog.common.utils.FileValidationUtils;

+                // 验证图片文件（多模态支持）
+                try {
+                    FileValidationUtils.validateImageFile(file);
+                } catch (IllegalArgumentException e) {
+                    log.warn("图片文件验证失败: {}", e.getMessage());
+                    emitter.completeWithError(new RuntimeException("图片文件验证失败: " + e.getMessage()));
+                    return emitter;
+                }
```

### 修改说明
- **新增**: `FileValidationUtils` 工具类
- **配置**: 文件大小限制（100MB文档，10MB图片）
- **代码**: 在Controller和Service层添加验证调用

---

## P0-4: SSE连接异常处理

### 修改文件
- `src/main/java/com/tripdog/service/impl/ChatServiceImpl.java`

### 具体修改
```diff
-        SseEmitter emitter = new SseEmitter(-1L);
+        // 设置SSE连接超时时间（30分钟）
+        SseEmitter emitter = new SseEmitter(30 * 60 * 1000L);

+            // 添加超时处理回调
+            emitter.onTimeout(() -> {
+                log.warn("SSE连接超时: conversationId={}, roleId={}, userId={}", 
+                    conversation.getConversationId(), roleId, userId);
+                try {
+                    emitter.complete();
+                } catch (Exception e) {
+                    log.debug("SSE连接已关闭，无法完成超时处理", e);
+                }
+            });
+            
+            // 添加完成回调（正常完成或异常完成）
+            emitter.onCompletion(() -> {
+                log.debug("SSE连接已完成: conversationId={}, roleId={}, userId={}", 
+                    conversation.getConversationId(), roleId, userId);
+                ThreadLocalUtils.remove(ROLE_ID);
+            });
+            
+            // 添加错误回调
+            emitter.onError((ex) -> {
+                log.error("SSE连接发生错误: conversationId={}, roleId={}, userId={}, error={}", 
+                    conversation.getConversationId(), roleId, userId, ex.getMessage(), ex);
+                ThreadLocalUtils.remove(ROLE_ID);
+            });
```

### 修改说明
- **修改**: SSE超时时间从无限制改为30分钟
- **添加**: `onTimeout()` 回调处理
- **添加**: `onCompletion()` 回调处理
- **添加**: `onError()` 回调处理
- **改进**: 异常处理中的资源释放

---

## P0-5: 临时文件清理机制

### 新增文件
- `src/main/java/com/tripdog/service/impl/TempFileCleanupService.java` (新建)

### 修改文件
- `src/main/java/com/tripdog/controller/DocController.java`

### DocController修改
```diff
            } finally {
-                // 清理本地临时文件
-                FileUploadUtils.deleteLocalFile(localFile);
+                // 清理本地临时文件（确保清理失败不影响主流程，但记录日志）
+                try {
+                    FileUploadUtils.deleteLocalFile(localFile);
+                    log.debug("临时文件清理成功: {}", localFile.getAbsolutePath());
+                } catch (Exception e) {
+                    // 清理失败不影响主流程，但记录警告日志
+                    log.warn("临时文件清理失败，文件可能残留: file={}, error={}", 
+                        localFile.getAbsolutePath(), e.getMessage());
+                    // 记录到待清理列表，由定时任务补偿清理
+                    // 注意：这里不抛出异常，避免影响主流程
+                }
            }
```

### TempFileCleanupService新增
- 定时任务：每6小时执行一次
- 清理逻辑：清理超过24小时的临时文件
- 文件识别：通过 `tripdog_` 前缀识别

### 修改说明
- **新增**: `TempFileCleanupService` 定时清理服务
- **改进**: Controller中的异常处理
- **添加**: 清理失败时的日志记录

---

## 📊 修改统计

| 修复项 | 修改文件数 | 新增文件数 | 代码行数变化 |
|-------|-----------|-----------|-------------|
| P0-1 | 1 | 0 | +2 |
| P0-2 | 1 | 0 | ~100行修改 |
| P0-3 | 3 | 1 | ~200行新增 |
| P0-4 | 1 | 0 | ~30行修改 |
| P0-5 | 1 | 1 | ~100行新增 |
| **总计** | **7** | **2** | **~430行** |

---

## ⚠️ 当前状态

- ✅ 代码修改已完成
- ⚠️ 测试覆盖不足（仅有结构验证）
- ⚠️ 未提交到Git
- ⚠️ 需要真实场景测试验证

